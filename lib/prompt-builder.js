...
<CharacterMap>
Break down EACH recognized Manchu word/character group. For every item, show:
  Manchu script → Romanization → Chinese equivalent → English meaning

Format as a table-like list, one entry per line:
  ᡥᠠᡶᠠᠨ → hafan → 官 (guān) → official
  ᡩᠣᡵᡤᡳ → dorgi → 内 (nèi) → inner, within
  ...

Include ALL characters found in the document, in reading order. Mark uncertain readings with (?).
</CharacterMap>
...
<ChineseText>
If Chinese text is present, transcribe it and provide its English translation. Note its relationship to the Manchu text (e.g., parallel translation, title, annotation). Highlight any differences between the Manchu and Chinese versions.
</ChineseText>
